...
// ─── Demo group fixtures (Screen 04 — built next commit) ──────────────────────
// One demo group per persona. Members + 3 pre-existing expenses each.
...
export interface DemoGroup {
  ...
  prefilledExpensePaidBy: string; // member ID
}
...
export const DEMO_GROUPS: Record<Persona, DemoGroup> = {
  roommate: {
    ...
    expenses: [
      { id: "e1", description: "August rent",     amount: 1650.00, paidById: "m-you",    splitAmongIds: ["m-you","m-priya","m-marcus"], date: iso(12), note: "split 3 ways" },
      { id: "e2", description: "Internet (Aug)",  amount: 90.00,   paidById: "m-priya",  splitAmongIds: ["m-you","m-priya","m-marcus"], date: iso(9),  note: "split 3 ways" },
      { id: "e3", description: "Costco run",      amount: 187.40,  paidById: "m-marcus", splitAmongIds: ["m-you","m-priya","m-marcus"], date: iso(4),  note: "split 3 ways" },
    ],
    balanceRibbonText: "You're owed $1,041.13",
  ...
  trip: {
    ...
    expenses: [
      { id: "e1", description: "Airbnb (4 nights)",  amount: 560.00, paidById: "m-you",  splitAmongIds: ["m-you","m-aryan","m-maya","m-sam"], date: iso(10), note: "split 4 ways" },
      { id: "e2", description: "Train to Sintra",    amount: 48.00,  paidById: "m-aryan",splitAmongIds: ["m-you","m-aryan","m-maya","m-sam"], date: iso(7),  note: "split 4 ways" },
      { id: "e3", description: "Beachside lunch",    amount: 94.00,  paidById: "m-you",  splitAmongIds: ["m-you","m-maya"],                    date: iso(3),  note: "you + Maya" },
    ],
    balanceRibbonText: "You're owed $467.00",
    balanceRibbonTone: "owed",
  ...
  couple: {
    ...
    expenses: [
      { id: "e1", description: "Aug groceries",          amount: 124.40, paidById: "m-you",    splitAmongIds: ["m-you","m-jordan"], date: iso(8), note: "split equally" },
      { id: "e2", description: "Hinge date (cocktails)", amount: 58.00,  paidById: "m-you",    splitAmongIds: ["m-you","m-jordan"], date: iso(5), note: "split equally" },
      { id: "e3", description: "Friday takeout",         amount: 42.00,  paidById: "m-jordan", splitAmongIds: ["m-you","m-jordan"], date: iso(2), note: "split equally" },
    ],
    balanceRibbonText: "You're owed $20.20",
    balanceRibbonTone: "owed",
...
export interface ReceiptItem {
  ...
  name: string;
  price: number;
  // suggested initial assignment — gets pre-applied so the user doesn't face
  // a blank assignment screen. They can change anything.
  defaultAssignedTo: string[]; // member IDs
}
